...
write_agent() {
  ...
  write_json "$AGENT_FILE" "$(python3 - "$current" <<'PY'
import json,os,sys,time
print(json.dumps({'pid':os.getppid(),'last_seen_at_unix':time.time(),'current_commit':sys.argv[1],'agent_version':1},ensure_ascii=False))
PY
)"
}
...
phase_from_line() {
  ...
  case "$line" in
    *"拉取 GitHub 最新"*) write_status running "同步 GitHub 源码" 10 "$line" "$request_id" "$current" "$target" ;;
...
process_request() {
  ...
  write_status running "准备更新" 3 "主机更新代理已接管任务" "$request_id" "$current" "$target"
  : > "$LOG_FILE"
...
if [[ ! -f "$STATUS_FILE" ]]; then
  current="$(git -C "$REPO_DIR" rev-parse HEAD 2>/dev/null || true)"
  write_status idle "等待更新" 0 "主机更新代理在线" "" "$current" "$current"
fi
