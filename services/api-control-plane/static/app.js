...
const CONSOLE_TIME_FORMATTER=new Intl.DateTimeFormat("zh-CN",{timeZone:CONSOLE_TIME_ZONE,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit",hourCycle:"h23"});
function cnTime(value,fallback="-"){
  if(!value)return fallback;
  const d=new Date(value);
  if(Number.isNaN(d.getTime()))return String(value);
  const parts={};
  CONSOLE_TIME_FORMATTER.formatToParts(d).forEach(part=>{if(part.type!=="literal")parts[part.type]=part.value});
...
async function loadDashboard(){
  ...
  $("dashboardTests").innerHTML=d.recent_tests.length?`<table><thead><tr><th>供应商</th><th>模式</th><th>状态</th><th>时间（北京时间）</th></tr></thead><tbody>${d.recent_tests.slice(0,8).map(t=>`<tr><td>${esc(t.provider_name)}</td><td>${esc(t.mode)}</td><td>${badge(t.status,statusKind(t.status))}</td><td>${esc(cnTime(t.finished_at||t.created_at,""))}</td></tr>`).join("")}</tbody></table>`:`<div class="empty">暂无测试记录</div>`;
  $("requestLogs").innerHTML=d.recent_requests.length?`<table><thead><tr><th>时间（北京时间）</th><th>客户端</th><th>类型</th><th>供应商</th><th>模型</th><th>协议</th><th>耗时</th><th>结果</th></tr></thead><tbody>${d.recent_requests.map(r=>`<tr><td>${esc(cnTime(r.created_at,""))}</td><td>${esc(r.client_name||"-")}</td><td>${esc(r.request_kind)}</td><td>${esc(r.provider_name||"-")}</td><td>${esc(r.resolved_model||"-")}</td><td>${badge(r.protocol||"-","blue")}</td><td>${r.latency_ms}ms</td><td>${r.success?badge("成功","good"):badge("失败","bad")}</td></tr>`).join("")}</tbody></table>`:`<div class="empty">Bot 尚未通过网关发起调用</div>`;
}
...
async function showTest(id){
  try{const t=await api(`/api/admin/tests/${id}`);const box=$("testDetail");box.classList.remove("hidden");
    ...
    const r=t.result;box.innerHTML=`<div class="panel-head"><div><h2>测试 #${id} 结果</h2><p>${esc(t.provider_name)} · ${esc(cnTime(r.started_at,"-"))} 至 ${esc(cnTime(r.finished_at,"-"))}（北京时间）</p></div><div class="actions"><button onclick="window.open('/api/admin/tests/${id}/report')">下载中文报告</button><button onclick="window.open('/api/admin/tests/${id}/raw')">下载原始JSON</button></div></div>
...
async function loadClients(){
  ...
  $("clientsTable").innerHTML=state.clients.length?`<table><thead><tr><th>名称</th><th>令牌前缀</th><th>状态</th><th>创建时间（北京时间）</th><th>最近使用（北京时间）</th><th>操作</th></tr></thead><tbody>${state.clients.map(c=>`<tr><td>${esc(c.name)}</td><td><code>${esc(c.token_prefix)}...</code></td><td>${c.enabled?badge("启用","good"):badge("停用","bad")}</td><td>${esc(cnTime(c.created_at,""))}</td><td>${esc(cnTime(c.last_used_at,"-"))}</td><td><div class="actions"><button onclick="toggleClient(${c.id})">${c.enabled?"停用":"启用"}</button><button onclick="deleteClient(${c.id})">删除</button></div></td></tr>`).join("")}</tbody></table>`:`<div class="empty">尚未创建 Bot 客户端令牌</div>`;
}
