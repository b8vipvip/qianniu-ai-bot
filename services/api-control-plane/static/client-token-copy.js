...
async function loadClients(){
  state.clients=await api("/api/admin/bot-web/clients");
...
async function createClient(){
  ...
  try{const r=await api("/api/admin/bot-web/clients",{method:"POST",body:JSON.stringify({name})});const copied=await copyTextCompat(r.token);alert(`客户端令牌：\n\n${r.token}\n\n${copied?"已复制到剪贴板。":"请手动复制并填入 Windows Bot。"}`);await loadClients();await loadDashboard()}catch(err){toast(err.message)}
}
async function copyClientToken(id){
  try{const r=await api(`/api/admin/bot-web/clients/${id}/token`);const copied=await copyTextCompat(r.token);toast(copied?"完整客户端令牌已复制":"复制失败，请检查浏览器剪贴板权限")}catch(err){toast(err.message)}
}
async function rotateClientToken(id){
  if(!confirm("重新生成后，旧令牌会立即失效，Windows Bot 必须填写新令牌才能重新连接。确认继续？"))return;
  try{const r=await api(`/api/admin/bot-web/clients/${id}/rotate`,{method:"POST"});const copied=await copyTextCompat(r.token);alert(`新客户端令牌：\n\n${r.token}\n\n${copied?"已复制到剪贴板。":"请立即手动保存。"}`);await loadClients();await loadDashboard()}catch(err){toast(err.message)}
}
