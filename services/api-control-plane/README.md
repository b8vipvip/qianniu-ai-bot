...
企业微信业务参数不再写入 `.env`。管理员登录控制面后，打开左侧“企业微信”页面配置：

- CorpID、应用 Secret、AgentId；
- 接收成员 UserID 与允许回复工单的成员；
- 回调 Token、43 位 EncodingAESKey 与工单有效期；
- 保存后可直接发送测试应用消息。

应用 Secret、回调 Token 和 EncodingAESKey 使用 `API_KEY_ENCRYPTION_KEY` 加密后保存在 `/data/api-control-plane.db`。管理接口只返回“是否已配置”，不会回显明文；网页保存后立即加载到现有企业微信桥接链路，无需重启容器。

多个成员 UserID 使用 `|` 分隔。允许回复成员留空时，仅允许接收成员回复。

### 2. 企业微信后台配置接收消息

先在控制面“企业微信”页面生成或填写回调参数并保存，再进入自建应用的“接收消息”设置，填写：

```text
URL: https://你的域名/api/wecom/callback
Token: 与控制面保存的 Token 完全相同
EncodingAESKey: 与控制面保存的 EncodingAESKey 完全相同
```

备份时必须同时保留 SQLite 数据目录和长期固定的 `API_KEY_ENCRYPTION_KEY`。
...
.env 中长期固定的 API_KEY_ENCRYPTION_KEY
```

只备份数据库但丢失加密密钥将无法恢复上游 ApiKey 和企业微信敏感配置。
