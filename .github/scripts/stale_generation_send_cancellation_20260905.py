...
# Initial send and retry confirmation are both generation-bound.
if qn.count('if (!await EnsureActiveBuyerForSendAsync(buyer))') != 2:
    raise SystemExit("thread token through buyer confirmation: unexpected call count")
qn = qn.replace(
...
qn = replace_once(
    ...
    '''                    cancellationToken.ThrowIfCancellationRequested();\n                    ok = await SendTextAsync(buyer, text);\n                    if (!ok && rpa.LastSendWasCancelled)''',
    "guard retry real send")
if qn.count('WaitForSellerEchoGraceAsync(buyer, text, sendStartedAt, 900)') != 2:
    raise SystemExit("seller-echo 900ms waits: unexpected call count")
qn = qn.replace(
    'WaitForSellerEchoGraceAsync(buyer, text, sendStartedAt, 900)',
    'WaitForSellerEchoGraceAsync(buyer, text, sendStartedAt, 900, cancellationToken)')
qn = replace_once(
    qn,
    'WaitForSellerEchoGraceAsync(buyer, text, sendStartedAt, 1400)',
    'WaitForSellerEchoGraceAsync(buyer, text, sendStartedAt, 1400, cancellationToken)',
...
# replies intentionally remain state-neutral because a first-inquiry greeting may
# continue into the AI path; cancellation therefore happens inside reliable send.
...
# retry delays and any not-yet-started composer/send-button side effects.
...
# 4) Static regression coverage for the production regression observed in
# 1.1.1280: generation 14 expired at ~55s but entered automatic UI send later.
