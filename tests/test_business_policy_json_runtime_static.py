...
def test_screenshot_target_question_blocks_purchase_answer():
    data = policy()
    p = data["patterns"]
    question = "那拍哪个链接啊"
    wrong = "拍电视端/大屏VIP链接即可，先下单后联系客服确认。"
    assert re.search(p["screenshotTargetQuestion"], question, re.I)
    assert re.search(p["badScreenshotTargetAnswer"], wrong, re.I)
    assert "不是拍商品链接" in data["prompts"]["screenshotTarget"]
