...
namespace Bot.Options
{
    public partial class WndOption : EtWindow
    {
        ...
        {
            get { return GetSelectedSettingTabControl(tabMain); }
        }
        ...
        public static void MyShow(string seller, WndAssist owner = null, OptionEnum showPage = OptionEnum.Unknown, Action uiCallback = null)
        {
            ...
            var wndOp = ShowSameNickOneInstance<WndOption>(seller, () => new WndOption(seller), owner, true);
            if (uiCallback != null)
            {
                wndOp.Closed += (s, e) =>
                {
        ...
        {
            TraversalOpsAndDoAction(op => op.RestoreDefault());
        }
        ...
        private void Save(string seller)
        {
            Util.Assert(!string.IsNullOrEmpty(seller));
            Hide();
            TraversalOpsAndDoAction(op => op.Save(seller));
            Close();
        }
