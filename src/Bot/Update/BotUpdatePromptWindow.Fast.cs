...
namespace Bot.UpdateNs
{
    internal sealed class BotUpdatePromptWindow : Window
    {
        ...
        public BotUpdatePromptWindow(BotReleaseInfo release)
        {
            ...
            {
                Text = "当前版本："
                    + BotUpdateService.CurrentVersion
                    + "    最新版本："
                    + release.Version
            ...
            {
                Content = BotUpdateService.IsPackageReady(release)
                    ? "立即安装并重启"
                    : "立即更新",
                MinWidth = 120,
                Height = 32,
                Margin = new Thickness(8, 0, 0, 0),
                IsEnabled = !string.IsNullOrWhiteSpace(
                    release.Sha256),
                Background = new SolidColorBrush(
        ...
        private async Task InstallAsync()
        {
            ...
            try
            {
                ...
                var package =
                    await BotUpdateService.DownloadPackageAsync(
                        _release,
                        progress,
                        _downloadCts.Token);
                _status.Text =
                    "安装包校验成功，正在启动更新程序...";
                BotUpdateService.LaunchInstaller(
                    package,
                    _release);
            }
        ...
        private void ResetButtons()
        {
            _installButton.IsEnabled =
                !string.IsNullOrWhiteSpace(_release.Sha256);
