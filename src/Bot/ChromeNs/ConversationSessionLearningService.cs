    ...
    internal static class ConversationSessionLearningService
    {
        ...
        public static void Initialize()
        {
            ...
            Log.Info("接待结束自动学习已启用：买家连续 " + InactivityMinutes
                + " 分钟无新消息后，自动复盘本轮人工与Bot回复并迭代知识和店铺表达风格。");
        }
        ...
        private static async Task FinalizeSessionAsync(ActiveSession session)
        {
            ...
            try
            {
                ...
                {
                    await Task.Run(() => ConversationContextStore.RefreshRemoteHistoryForLearning(
                        session.Seller,
                        session.Buyer,
                        session.CCode));
                ...
                var turns = ConversationContextStore.GetTurnsBetween(
                    session.Seller,
                    session.Buyer,
                ...
                if (analysis == null)
                {
                    CompleteWithoutLearning(session, "AI未返回可解析的复盘结果，未修改知识库。", "复盘结果为空");
                    return;
                }
                ...
                foreach (var suggestion in parsed.Take(MaxSuggestionsPerSession))
                {
                    if (!ShouldApply(suggestion))
                    {
                        ...
                        suggestion.ApplyMessage = string.IsNullOrWhiteSpace(suggestion.ApplyMessage)
                            ? "未满足自动学习安全条件"
                            : suggestion.ApplyMessage;
                        skipped++;
                        continue;
                    }
        ...
        {
            var transcript = BuildTranscript(turns, botCards);
            var cards = BuildBotCards(botCards);
            var messages = new JArray
            {
                new JObject
                {
                    ...
                    ["content"] =
                        ...
                        + "规则：1. Bot自身标记为[Bot]的回复不能作为新增事实的唯一证据，禁止自我学习、自我强化；"
                        + "2. 客服撤回的消息不是有效答案，但Bot被撤回后人工重新发送的内容属于强修正证据；"
                        + "3. 人工客服最终有效回复优先于Bot旧答案；如果人工只是补充语气而没有改变事实，可优化表达但不得编造新事实；"
            ...
            List<BotConversationHistoryEntity> botCards)
        {
            ...
            foreach (var turn in turns.OrderBy(x => x.Timestamp))
            {
                ...
                else if (turn.Withdrawn)
                {
                    role = "客服-已撤回";
                }
                else if (IsBotTurn(turn, botCards))
                {
                    role = "Bot";
                }
                else
        ...
        private static void SaveStyleProfile(string seller, string profile)
        {
            ...
            EnsureSchema();
            StoreReplyStyleProfileEntity existing = null;
            try
            {
                lock (DbSync)
                {
                    var rows = DbHelper.Db.Select(
                        ...
                        "where Seller = ? limit 1",
                        seller);
                    existing = (rows ?? new List<object>())
                        .OfType<StoreReplyStyleProfileEntity>()
                        .FirstOrDefault();
                    var entity = existing ?? new StoreReplyStyleProfileEntity { Seller = seller };
        ...
        private static void DisposeAllTimers()
        {
            foreach (var session in Active.Values)
            {
                try
                {
                    if (session.Timer != null) session.Timer.Dispose();
                }
                catch
                {
                }
