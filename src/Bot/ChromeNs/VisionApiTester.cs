...
namespace Bot.ChromeNs
{
    internal static class VisionApiTester
    {
        ...
        {
            if (endpoint == null || !endpoint.SupportsVision || string.IsNullOrWhiteSpace(endpoint.VisionModel)) return new ApiEndpointTestResult { Success = false, ShortStatus = "未配置视觉模型", DisplayText = "失败：请先启用图片视觉理解并填写 VisionModel。" };
